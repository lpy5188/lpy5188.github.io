...
{
  let textInputBox = document.getElementById("txt-search");
  let searchKeyword = textInputBox.value;
  textInputBox.value = "";
  ...
  let searchResult = searchByDescription(searchKeyword);
...
function showVideos(vList)
{
  ...
  {
    resultDiv.innerHTML += generateRowHTML(vList[j]);
  }
}

function showNoResult(keywordText)
{
  let resultDiv = document.getElementById("results");
  resultDiv.innerHTML = "No videos found with the keyword " + keywordText;
}

function searchByDescription(keywordText)
{
  let result=[];
  let text = keywordText.toLowerCase();

  let i = 0;
  for (i = 0; i < videosList.length; i++)
  {
    let videoObject = videosList[i];
    let description =  videoObject["description"].toLowerCase();
  ...
  console.log("matched total = " + result.length);
